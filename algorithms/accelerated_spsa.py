...
class Accelerated_SPSA:
    def __init__(
        ...
        self.alpha_base: float = 0.3
        self.gamma_base: float = 0.2
        self.momentum: float = 0.95
        self.b: int = 1
    ...
    def run_main_algorithm(self) -> Dict[int, Dict[int, np.ndarray]]:
        ...
        y_hat: Dict[int, Dict[int, np.ndarray]] = {
            target_id: {
                sensor_id: self.init_coords[target_id][sensor_id].copy()
                for sensor_id in self.sensor_ids
            }
        ...
        for l in self.target_ids:
            ...
            for i in self.sensor_ids:
                ...
                x1: np.ndarray = y_hat[l][i] + self.beta_1 * delta
                x2: np.ndarray = y_hat[l][i] - self.beta_2 * delta
                ...
                theta_diff: List[np.ndarray] = [
                    abs(b[j]) * (y_hat[l][i] - y_hat[l][j]) for j in neighbors_i
                ]

                adaptive_factor = 1.0 / (1 + 0.005)
                current_alpha = self.alpha_base * adaptive_factor
                current_gamma = self.gamma_base * adaptive_factor
